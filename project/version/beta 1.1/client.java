...
class ControlClientThread extends Thread
{
    ...
    void checkClientFiles()
    {
        ...
        for (int i = 0; i < serverFilesForUser.size(); i++) {
            if (clientFileNames.indexOf(serverFilesForUser.get(i)) == -1) {
                ...
                int index = serverFilesPaths.indexOf(serverFilesForUser.get(i));
    ...
    {
        List<Integer> csvFiles = new ArrayList<Integer>();
        //int iterator = 0;
        String line;
        String[] csvRecords;
        ...
        for(int i=0; i < serverFileNames.size(); i++) {
            //System.out.println(serverFileNames.get(i));
            if (serverFileNames.get(i).equals("data.csv")) {
                csvFiles.add(i);
                System.out.println("dziala raz");
            }
        }

        for(int i = 0; i < csvFiles.size(); i++) {
            try {
                BufferedReader bufferedReader = new BufferedReader(new FileReader(serverFilesPaths.get(i)));

                while ((line = bufferedReader.readLine()) != null) {

                    csvRecords = line.split(",");
                    if(csvRecords[0].equals(userName))
                        serverFilesForUser.add(csvRecords[1]);
                }
    ...
    public void run()
    {
        ...
        while (true)
        {
            ...
            if(readServerRecords())
                System.out.println("read records problem");
            checkClientFiles();
            //System.out.println(serverFilesPaths.get(4));
            // sleep
            delay(1000);
           ...
           checkServerFiles();
            ...
            System.out.println("servera:  " + serverFilesForUser);
